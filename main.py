...
def get_current_user():
    ...
    # First check if auth_user is cached in session state (most reliable)
    if st.session_state.get('auth_user'):
        return st.session_state.get('auth_user')
    # Fallback: verify token if available
...
def render_saved_searches_page():
    ...
    if not saved_searches:
        st.info("No saved searches yet! Start by saving a search from the main page.")
        return
...
def render_save_search_widget(filters: dict, keywords: str = None):
    ...
    user = get_current_user()
    if not user:
        st.warning("Please log in to save searches")
        return
    ...
    with st.expander("💾 Save This Search"):
        ...
        if st.button("Save", key=f"save_btn_{id(filters)}", use_container_width=True):
            if search_name:
                success, message, sid = create_saved_search(user_id, search_name, filters, keywords)
                if success:
                    st.success(f"{message}")
                    st.session_state['last_saved_search_id'] = sid
                else:
                    st.error(message)
            else:
...
def generate_comparison_pdf(data_dict):
    ...
    # FPDF.output(dest='S') returns a str in this environment; encode to bytes for downloads
...
# ============================================================================
# LOGO COMPRESSION & RESIZING HELPER
# ============================================================================
def compress_and_resize_logo(img_path: str, max_width: int = 1000, quality: int = 85) -> bytes:
    """Load, compress, resize logo and return as bytes for display."""
    try:
        img = Image.open(img_path)
        # Resize if width > max_width while preserving aspect ratio
...
# Display compressed and resized logo
logo_path = r"C:\Users\user\Desktop\idk\nouma finds plan.jpg"
logo_bytes = compress_and_resize_logo(logo_path, max_width=2000, quality=85)
...
if search_vector:
    try:
        ...
        # Helper: apply defensive client-side filtering in case server-side filter didn't match exactly
        def _payload_matches_filters(payload):
            # Category
            if category_filter and category_filter != "Tous":
                pcat = payload.get('category', '')
                if isinstance(pcat, (list, tuple)):
                    if not any(str(category_filter).strip().lower() == str(x).strip().lower() for x in pcat if x is not None):
                        return False
                else:
                    if str(category_filter).strip().lower() != str(pcat).strip().lower():
                        return False
            # Color
            if color_filter and color_filter != "Toutes":
                pcol = payload.get('color', '')
                if isinstance(pcol, (list, tuple)):
                    if not any(str(color_filter).strip().lower() == str(x).strip().lower() for x in pcol if x is not None):
                        return False
                else:
                    if str(color_filter).strip().lower() != str(pcol).strip().lower():
                        return False
            # Budget/price
            try:
                price_val = float(payload.get('price', payload.get('display_price', budget)))
            except Exception:
                price_val = float(budget)
            if price_val > float(budget):
                return False
            return True

        for point in scroll_result[0]:  # scroll returns (points, next_offset)
            # Skip points that don't match filters (defensive)
            try:
                if not _payload_matches_filters(point.payload):
                    continue
            except Exception:
                # If payload inspection fails, skip the point conservatively
                continue
...
<div style='margin-top: 1rem; text-align: center;'>
  <h4 style='color: #1A2B4A; margin-bottom: 0.5rem;'>  °˖✧◝(⁰▿⁰)◜✧˖°  </h4>
</div>
""", unsafe_allow_html=True)

# Render 3x3 sponsor images. Place images in a local `sponsors/` folder named sponsor1..sponsor9.png
sponsor_links = [
    "https://qdrant.tech/",
...
# Optional friendly names for captions under each sponsor image
sponsor_names = [
    "Qdrant",
    "Edusmart",
    "InstaDeep",
...
idx = 1
# Build a responsive 3x3 CSS grid for sponsors so images align uniformly
sponsor_items = []
for i in range(1, 10):
        ...
        if data_uri:
                ...
                src = f"https://via.placeholder.com/300x120?text=Sponsor+{i}"
...
'''<style>
    ...
    @media (max-width: 600px) {
        ...
        .sponsor-item img { height: 72px; }
        .sponsor-item figcaption { font-size: 0.7rem; }
    }
</style>
